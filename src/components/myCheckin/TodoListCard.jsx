...
const TodoListCard = ({ initMyCheckInOkrTaskList, bpList }) => {
    ...
    const handleBpChange = useCallback((value) => {
        setBpNo(value);
        setTaskLimitStart(0);
        setHasMore(true);
        fetchToDoList(0, selectedOtStat, value);
    }, [selectedOtStat, fetchToDoList]);
    ...
    const lastElementRef = useCallback(node => {
        ...
        observer.current = new IntersectionObserver(entries => {
            if (entries[0].isIntersecting) {
                const newLimitStart = taskLimitStart + 10;
                fetchToDoList(newLimitStart, selectedOtStat, bpNo);
                setTaskLimitStart(newLimitStart);
            }
        });
        if (node) observer.current.observe(node);
    }, [loading, hasMore, selectedOtStat, bpNo, fetchToDoList, taskLimitStart]);
