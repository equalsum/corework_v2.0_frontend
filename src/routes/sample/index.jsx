...
import EgovMain from 'pages/sample/main/Main';
...
//ADMIN
import EgovAdminScheduleDetail from 'pages/admin/schedule/AdminScheduleDetail';
import EgovAdminScheduleEdit from 'pages/admin/schedule/AdminScheduleEdit';
import EgovAdminScheduleList from 'pages/admin/schedule/AdminScheduleList';

import EgovAdminBoardEdit from 'pages/admin/board/AdminBoardEdit';
import EgovAdminBoardList from 'pages/admin/board/AdminBoardList';

import EgovAdminUsageEdit from 'pages/admin/usage/AdminUsageEdit';
import AdminUsageList from 'pages/admin/usage/AdminUsageList';

import EgovAdminNoticeDetail from 'pages/admin/notice/AdminNoticeDetail';
import EgovAdminNoticeEdit from 'pages/admin/notice/AdminNoticeEdit';
import EgovAdminNoticeList from 'pages/admin/notice/AdminNoticeList';

import EgovAdminGalleryDetail from 'pages/admin/gallery/AdminGalleryDetail';
import EgovAdminGalleryEdit from 'pages/admin/gallery/AdminGalleryEdit';
import EgovAdminGalleryList from 'pages/admin/gallery/AdminGalleryList';
//사이트관리자 암호 바꾸기 기능 추가 2023.04.15(토) 김일국 추가
import * as EgovNet from 'api/Fetch'; // jwt토큰 위조 검사 때문에 추가
import initPage from 'js/ui';
import EgovAdminPasswordUpdate from 'pages/admin/manager/AdminPasswordUpdate';
...
const RootRoutes = () => {
    ...
    useEffect(() => {
        if (!isMounted.current) { // 컴포넌트 최초 마운트 시 페이지 진입 전(렌더링 전) 실행
            ...
            setMounted(true); // 이 값으로 true 일 때만 페이지를 렌더링이 되는 변수 사용.
            const regex = /^((?!login).)*$/; //정규표현식 사용: /admin/~ 으로 시작하는 경로 모두 포함
    ...
    if(mounted) { // 인증 없이 시스템관리 URL로 접근할 때 렌더링 되는 것을 방지하는 조건추가.
        return (
            <Routes>
                ...
                <Route path={URL.LOGIN} element={<Login onChangeLogin={(user) => setLoginVO(user)} />}/>
    ...
    return (
        <>
            ...
            <Routes>
                ...
                <Route path={URL.ADMIN_USAGE} element={<AdminUsageList />} />
